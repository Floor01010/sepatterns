...
public class GameOfLife extends JPanel {
	...
	private void advance() {
		...
		for (int i=0; i<grid.length; i++)
			for (int j=0; j<grid[0].length; j++)
				if (grid[i][j].getState() && (nbrOfNeighbors(i,j) < 2))
					newgrid[i][j] = false;
				else if (grid[i][j].getState() && (2 <= nbrOfNeighbors(i,j)) && (nbrOfNeighbors(i,j) <= 3))
					newgrid[i][j] = true;
				else if (grid[i][j].getState() && (3 < nbrOfNeighbors(i,j)))
					newgrid[i][j] = false;
				else if (!grid[i][j].getState() && (nbrOfNeighbors(i,j) == 3))
					newgrid[i][j] = true;
